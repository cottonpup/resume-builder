...
export function FullPreviewFooter(props: Props) {
    return (
        <div className="bg-black p-[4px] absolute left-2/4 -translate-x-2/4 bottom-[16px] flex items-center text-white fill-white rounded-[18px]">
            <button onClick={() => props.updatePagination({ type: 'previousPage' })}>
                <svg
            ...
            <div className="text-[12px] mx-[4px]">{`${props.pagination.currentPage} / ${props.pagination.totalPages}`}</div>
            <button onClick={() => props.updatePagination({ type: 'nextPage' })}>
                <svg
                    width="24"
                    height="24"
                    viewBox="0 0 24 24"
