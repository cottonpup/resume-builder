import { Dispatch } from "react";
import { PaginationAction, PaginationState } from "../hooks/usePagination";

interface Props {
  pagination: PaginationState;
  updatePagination: Dispatch<PaginationAction>;
}

export function SmallPreviewHeader(props: Props) {
  return (
    <div className="p-4 flex fill-[#ffffff] text-[#ffffff] items-center mb-2 z-10">
      <div onClick={() => props.updatePagination({type: 'previousPage'})}>
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
          className="rotate-180 opacity-[0.3]"
        >
          <path d="M9.32427537,7.23715414 L10.6757246,5.76284586 L16.6757246,11.2628459 C17.1080918,11.6591824 17.1080918,12.3408176 16.6757246,12.7371541 L10.6757246,18.2371541 L9.32427537,16.7628459 L14.5201072,12 L9.32427537,7.23715414 Z"></path>
        </svg>
      </div>
      <div className="text-sm">{`${props.pagination.currentPage} / ${props.pagination.totalPages}`}</div>
      <div onClick={() => props.updatePagination({type: 'nextPage'})}>
        <svg
          width="24"
          height="24"
          viewBox="0 0 24 24"
          version="1.1"
          xmlns="http://www.w3.org/2000/svg"
          className="opacity-[0.3]"
        >
          <path d="M9.32427537,7.23715414 L10.6757246,5.76284586 L16.6757246,11.2628459 C17.1080918,11.6591824 17.1080918,12.3408176 16.6757246,12.7371541 L10.6757246,18.2371541 L9.32427537,16.7628459 L14.5201072,12 L9.32427537,7.23715414 Z"></path>
        </svg>
      </div>
    </div>
  );
}
